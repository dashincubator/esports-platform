<?php
    ...
    if ($app['auth']->isGuest() && $app['route']->has('account.auth.reset-password')) {
        $app['modal']->add('@components/account/modals/reset-password');
    }
?>
...
<div class="container">
    ...
    <section class='index-header section section--margin-top-large'>
        <div class="page-header --max-width-1000px">
            <h1 class='index-header-title page-header-title'>
                <span class="--text-primary">Play</span> Games. <span class="--text-primary">Win</span> Money.
            </h1>
            <span class='page-header-subtitle --text-crop-bottom'>
                Play for fun in our free play <b class='--text-black'>Ladders</b>, add a wager to any match - no fee.
                Compete in our weekly <b class='--text-black'>Leagues</b> for cash and prizes, or battle through our <b class='--text-black'>Tournaments</b> to win it all.
            </span>
        </div>
    </section>

    <?php if ($app['auth']->isGuest()): ?>
        <section class='section section--margin-top --flex-center'>
            <div class='button-group --flex-horizontal-space-between --margin-top-large'>
                <div class='button-group-item button button--large button--primary button--width-large --margin-top-large --width-full-600px' data-click='modal' data-modal='sign'>
    ...
    <?php if (count($events)): ?>
                ...
                <div class="scroller-content">
                    <div class="scroller-content-wrapper">
                        <?php foreach($events as $slug => $event): ?>
                            <div class="index-game <?= $i === 0 ? '--active' : '' ?>" data-click='frame' data-frame='<?= $slug ?>'>
        ...
        <div class="frames frames--overflow">
            ...
            <?php foreach ($events as $slug => $group): ?>
                <div class="frame <?= $i === 0 ? '--active' : '' ?>" id='frame-<?= $slug ?>'>
                    <?php
                        $filter = [
                            'all' => 'All Platforms'
                        ];
                            ...
                            <div class="filter table-row table-row--more-right-1000px --active --background-grey-300 --margin-top-border" data-filter='<?= "{$slug}{$platform['id']}" ?>' data-ref='filter'>
                                <div class="table-item table-item--padding-right table-item--width-fill --flex-vertical --game-icons-small-left --game-icons-left-hidden-600px">
                                    ...
                                    <div class="text-list">
                                        ...
                                        <div class="text --text-small --text-truncate">
                                            <span>
